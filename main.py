...
S3_BUCKET_READ_DATA_PATH = 's3://bigdataprojectdatastorage/data/'
S3_OUTPUT_PATH = 's3://bigdataprojectdatastorage/DataVisualizationOutput/'
...
class FeatureEngineering:
    ...
    def combine_all_dataframes(self,*dfs):
        ...
        print("Total Number of Rows  are {}".format(combined.count()))
        return combined
...
class DataVisualization:
    ...
    def plot_most_desired_departure_location(self,sc,df):
        ...
        with s3.open(S3_OUTPUT_PATH + "Plotting Top 15 Most Desired Departure Location.png", 'wb') as f:
    ...
    def plot_top_10_most_desired_destination_states(self,sc,df,year):
        ...
        img_data.seek(0)
        s3 = s3fs.S3FileSystem(anon=False)  # Uses default credentials
        with s3.open(S3_OUTPUT_PATH + 'Top 10 Desired Destination States in {}.png'.format(year), 'wb') as f:
            f.write(img_data.getbuffer())
    ...
    def plot_most_desired_destination_customstate1_customestate2_each_year(self, sc, df,state1,state2):
        ...
        with s3.open(S3_OUTPUT_PATH + 'Line Plot to compare total flighs for 2 states.png', 'wb') as f:
            f.write(img_data.getbuffer())
        # fig.savefig('Line Plot to compare total flighs for 2 states.png', dpi=fig.dpi)
    ...
    def plot_histogram_for_outliers(self,arr,dep):
        ...
        #plt.savefig('Histogram of Outliers.png')
